...
{
    int points;
    if ((snd->number == fst->number + 1 || snd->number == fst->number - 1) && !isFlush){
        points = 5;
    }
    else if((snd->number == fst->number) && isFlush){
        points = 7;
    }
    else if((snd->number == fst->number + 1) && upwardsFlush){
        points = 3;
    }
    else if((snd->number == fst->number-1) && !upwardsFlush && isFlush){
        points = 3;
    }
    else if((snd->number == fst->number + 1) && isFlush && !upwardsFlush){
        points = 5;
    }
    else if ((snd->number == fst->number -1) && isFlush && upwardsFlush){
        points = 5;
    }
    else if((snd->number != fst->number +1 || snd->number != fst->number -1) && isFlush){
        points = -3;
    }
    else{
        points = 0;
